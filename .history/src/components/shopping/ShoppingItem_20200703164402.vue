  ...
  <div>
    ...
    <div class="wrapper" ref="wrapper">
      <div class="content">
        <div class="bottom" v-if="shopList.length>0">
          <van-cell-group>
            ...
            <van-cell>
              <template #default>
                <span class="m-r-1">
                  <van-button color="rgb(230,5,127)" size="small" @click="del">删除</van-button>
                </span>
                <van-button color="rgb(230,5,127)" size="small" @click="shopping">去结算</van-button>
              </template>
            </van-cell>
          </van-cell-group>
        ...
        <div class="f-j" v-if="shopList.length===0">
          <img class="image" src="../../assets/u=3842446086,2230018171&fm=193.jpg" alt="">
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    shopping() {
      ...
      // console.log(arr);
      if (arr.length > 0) {
        this.$router.push({ path: "/settlement", query: { arr,idDirect:false } });
      } else {
