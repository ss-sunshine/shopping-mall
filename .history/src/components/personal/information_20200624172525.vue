<template>
  <div>
    ...
    <van-cell-group>
      <!-- 输入任意文本 -->
      <van-field v-model="username" label="用户名" readonly/>
      <!-- 输入手机号，调起手机号键盘 -->
      <van-field v-model="name" label="昵称" />
      <!-- 允许输入正整数，调起纯数字键盘 -->
      <van-field v-model="gender" label="性别" />
      <!-- 允许输入数字，调起带符号的纯数字键盘 -->
      <van-field v-model="email" type="number" label="邮箱" />
      <!-- 输入密码 -->
      <van-field v-model="date" label="出生年月" />
    </van-cell-group>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    if (user !== null) {
      ...
      this.gender = user.userInfo.nickname;
      this.avatar = user.userInfo.nickname;
    }
  },
  watch: {},
  computed: {}
