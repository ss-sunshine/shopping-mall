<template>
  <div>
    ...
    <van-cell-group>
      <van-field label="昵称" v-model="user.username" readonly />
    </van-cell-group>
    <van-cell-group>
      <van-field v-model="user.nickname" label="昵称" placeholder="请输入用户名" />
      <van-field v-model="user.gender" label="性别" placeholder="请输入用户名" />
      <van-field v-model="user.email" type="number" label="邮箱" placeholder="请输入邮箱" />
...
export default {
  ...
  mounted() {
    this.$api
      ...
      this.date = this.user.year + "年" + this.user.month + "月" + this.user.day + "日";
  },
  watch: {},
  computed: {}
};
