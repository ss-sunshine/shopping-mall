...
export default {
  ...
  mounted() {
    ...
    this.$api
      .getGetAddress(this.id)
      .then(res => {
        console.log(res.address);
        this.addressInfo = {
          name: res.address[0].name,
          tel: res.address[0].tel,
          province: res.address[0].province,
          city: res.address[0].city,
          county: res.address[0].county,
          areaCode: res.address[0].areaCode,
          postalCode: res.address[0].postalCode,
          addressDetail: res.address[0].addressDetail
        };
      })
      .catch(err => {
        console.log(err);
