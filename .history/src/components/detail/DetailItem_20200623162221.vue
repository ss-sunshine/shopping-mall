<template>
  <div class="detail">
    ...
    <van-goods-action class="shop-icon">
      ...
      <van-goods-action-icon icon="cart-o" text="购物车" @click="shop"/>

      <van-button color="rgb(255,136,85)" square text="加入购物车" size="large" @click="addShop"/>
      <van-button color="rgb(255,68,68)" square text="立即购买" size="large" @click="addShop"/>
    </van-goods-action>
  </div>
</template>
...
export default {
  ...
  methods: {
    addShop(){
      this.$api.getAddShop({id:this.id}).then(res=>{
        console.log(res);
    ...
    no() {
      ...
      if (user !== null) {
        ...
        this.$api.getCollection(this.detail).then(res=>{
          // console.log(res);
        }).catch(err=>{
          console.log(err);
        })
    ...
    yes() {
        this.flag = false;
        this.text = "收藏：";
        this.$api.getCancelCollection({id:this.id}).then(res=>{
          // console.log(res);
        }).catch(err=>{
          console.log(err);
        })
