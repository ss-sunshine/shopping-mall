  ...
  <div>
    <div class="detail">
      ...
      <van-goods-action class="shop-icon">
        ...
        <van-button color="rgb(255,136,85)" square text="加入购物车" size="large" @click="checkaddShop" />
...
export default {
  ...
  methods: {
    ...
    Recently(){
      localStorage.setItem("Recently",this.id)
      let Recently = localStorage.getItem("Recently")
      this.Recently.push(Recently)
      this.RecentlyArr = this.Recently.filter((item, index, array) => {
        return array.indexOf(item) === index;
      });
      localStorage.setItem("Recently", JSON.stringify(this.RecentlyArr));
      this.RecentlyArr = JSON.parse(localStorage.getItem("Recently"));
