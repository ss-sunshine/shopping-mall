  ...
  methods: {
    clickItem(item) {
      ...
      if (this.idDirect === false) {
        let arr = localStorage.setItem("shopping", JSON.stringify(this.arr));
        this.$router.push({ path: "/settlement", query: { item,arr } });
      } else if (this.idDirect === true) {
        let arr = localStorage.setItem("detail", JSON.stringify(this.detail));
        this.$router.push({ path: "/settlement", query: { item,arr } });
      }
      // this.$router.back()
    },
