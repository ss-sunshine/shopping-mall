  ...
  <div class="ipt">
    ...
    @search="onSearch">
      <template #action>
        ...
        <div v-if="list.length!==0||flag===true" @click="on">取消</div>
      </template>
      <template #left>
    ...
    <div class="list" v-if="list.length===0&&flag===true">
      ...
      <div v-for="item in histoty" :key="item">
        <div class="histoty">{{item}}</div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    enter(){
      this.flag = true
      let histoty = localStorage.getItem('value')
      // console.log(histoty.split('"'))
      this.histoty=histoty.split('"')
