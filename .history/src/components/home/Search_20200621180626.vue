<template>
  <div class="ipt">
    ...
    <van-search v-model="value" show-action placeholder="请输入搜索关键词" @search="onSearch">
      <template #action>
        <div v-if="list.length===0" @click="onSearch">搜索</div>
    ...
    <!-- </form> -->
    <div  v-if="list.length!==0" :finished="finished" finished-text="没有更多了" @load="onSearch" class="list">
      <div v-for="item in list" :key="item.id">
        <!-- {{item.name}} -->
        <div class="item f14" @click="clickDetail">{{item.name}}</div>
        
      </div>
      <div class="item1 f14">没有更多了</div>
    </div>
  </div>
</template>
