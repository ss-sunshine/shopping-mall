<template>
  <div>
    ...
    <div v-if="searchCity === ''">
      <div class="now-city p-b-1 p-l-2 f14">当前城市</div>
      <div class="nowCityOUT p-tb-1 p-l-2">
        <div class="nowCity f-a-j f14">
          <div v-if="nowcity">{{nowcity}}</div>
          <div v-else>定位中...</div>
        </div>
      </div>
      <div class="now-city p-tb-1 p-l-2 f14">热门城市</div>
      <div class="d-f f-w m-b-10">
        <div v-for="(item,index) in hotCities" :key="index">
          <div
            class="hotCity f-a-j f14 f1 m-l-15 m-t-1"
