        ...
        <div v-if="searchCity === ''">
          ...
          <div class="d-f f-w m-b-1">
            <div v-for="(item,index) in hotCities" :key="index">
              <div class="hotCity f-a-j f14 f1 m-l-1 m-t-1" @click="checkCity(item)">{{item.name}}</div>
            </div>
          </div>
          <van-index-bar class="side_font top_index-bar margin_index-bar">
...
export default {
  ...
  mounted() {
    new BScroll(this.$refs.contenter, {
      click: true
    });
