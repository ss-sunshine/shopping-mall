<template>
  <div>
    ...
    <div class="wrapper" ref="wrapper">
      <div class="content">
        <van-cell-group v-for="(item,index) in recentlyArr" :key="item.id">
          <van-card :title="item.goodsName">
            <template #thumb>
              <img :src="item.image"  @click="Detail(item, index)"/>
            </template>
            <template #price>
              <div class="price f14 f-w-7">￥{{item.mallPrice}}</div>
            </template>
            <template #num>
...
export default {
  ...
  methods: {
    ...
    del(item, index) {
      this.id = item.id;
      this.list = this.recentlyArr.filter(item => {
        return this.id !== item.goodsId;
      });
      this.$toast("删除成功");
    }
  },
