<template>
  <div>
    ...
    <div>
      ...
      <van-cell-group>
        <van-field v-model="value" placeholder="说说你的购买感受吧~~" />
        <div class="uploader">
...
export default {
  ...
  methods: {
    submit() {
      this.$api.getComment({ id: this.obj.cid, rate:this.num,content:this.value,anonymous:this.checked,_id:this.obj._id,order_id:this.obj.order_id,image:this.obj.image_path });
    },
    afterRead(file) {
      // 此时可以自行将文件上传至服务器
      console.log(file);
