          ...
          <div v-if="activeName==='b'">
            <div v-for="(item,index) in arr" :key="index">
              <div  v-for="(item1,index1) in item.goods" :key="index1">
                ...
                <template #num>
                  <div class="f-a-j dryingsSheet f12"  @click="clickItem1(item1)">
                    <div>查看评价</div>
                  </div>
                </template>
              </van-card>
...
export default {
  ...
  methods: {
    ...
    clickItem1(item1) {
        localStorage.setItem('detailEvaluation',JSON.stringify(item1))
        this.$router.push({ path: "/detailEvaluation", query: { item1 } });
    },
  },
  mounted() {
